//Compile: g++ trim_test.cpp -lboost_unit_test_framework
...
void trim_test()
{
    ...
    str.clear();
    trim_left_copy_if(std::back_inserter(str), "     1x x x x1     ", is_space());
    BOOST_CHECK(str == "1x x x x1     ");

    str.clear();
    trim_right_copy_if(std::back_inserter(str), "     1x x x x1     ", is_space());
    BOOST_CHECK(str == "     1x x x x1");
    ...
    BOOST_CHECK(trim_copy_if(string("123abc456"), is_classified(std::ctype_base::digit)) == "abc");
    BOOST_CHECK(trim_copy_if(string("<>abc<>"), is_any_of("<<>>")) == "abc");
}


//trim left and right, and delete the repeat character in the middle, only keep the first
void trim_all_test()
{
    string str1("     1x   x   x   x1     ");
    string str2("+---...2x+--x--+x-+-x2...---+");
    ...
    // *** non-standard predicate tests *** //
    BOOST_CHECK(trim_all_copy_if(string("123abc127deb456"), is_classified(std::ctype_base::digit)) == "abc1deb");
    BOOST_CHECK(trim_all_copy_if(string("<>abc<>def<>"), is_any_of("<<>>")) == "abc<def");
    BOOST_CHECK(trim_all_copy_if(string("<>abc<>def<>"), is_any_of("<>")) == "abc<def");
}


//trim left and right, fill in middle
void trim_fill_test()
{
    string str1("     1x   x   x   x1     ");
    string str2("+---...2x+--x--+x-+-x2...---+");
    ...
    // *** non-standard predicate tests *** //
    BOOST_CHECK(trim_fill_copy_if(string("123abc127deb456"), "+", is_classified(std::ctype_base::digit)) == "abc+deb");
