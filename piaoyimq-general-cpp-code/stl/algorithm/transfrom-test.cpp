...
#include "../container/vector/print_vector.h"

#if 0
void test_transform1()
{
    // compare two sequences and produce a third one
    // having true for positions where both sequences
    // have equal elements
    std::vector<int> seq1 = { 1, 2, 3, 4 };
    std::vector<int> seq2 = { 2, 4, 3 };
    std::transform(seq1.begin(), seq1.end(), seq2.begin(), std::inserter(seq2.end()), std::equal_to<int>());
