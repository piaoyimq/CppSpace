...
socket_session::socket_session(boost::asio::io_service& io_srv) :
		m_io_service(io_srv), m_socket(io_srv), m_business_type(0), m_app_id(0)
{
...
void socket_session::handle_read_header(const boost::system::error_code& error)
{
	...
	try
	{
		...
		std::string data;
		data.swap(sBody);
		boost::asio::async_read(m_socket, boost::asio::buffer(sHeader),
...
void socket_session::handle_read_body(const boost::system::error_code& error)
{
	...
	try
	{
		...
		boost::asio::async_read(m_socket, boost::asio::buffer(pBody, dwLength),
...
void socket_session::handle_write(const boost::system::error_code& error, std::size_t bytes_transferred, std::string* pmsg)
{
	//数据发送成功就销毁
	if (pmsg != NULL)
	{
